...
bot = TeleBot(token='5940549620:AAF6YK-DgN0N0NDUHYuTd9U6kTw5R2ug22c', parse_mode='html') # создание бота
...
DEFINITOINS = {
    ...
    'тестирование': 'поиск разницы между ожидаемым результатом и фактическим',
    'баг':'несоответствие фактического результата ожидаемому',
    'тестировщик':'специалист, который ищет баг на ранних этапах и фиксирует его',
    'Клиент':'элемент клиент-серверной архитектуры, который отправляет и принимает от сервера HTTP-запросы',
    'Сервер':'элемент клиент-серверной архитектуры, который взаимодействует с клентом через HTTP-запросы, HTTP-ответы, и взаимодействует с БД через SQL-запросы, данные',
    'БД':'элемент клиент-серверной архитектуры, который получает от сервера SQL-запросы и отправляет данные',
